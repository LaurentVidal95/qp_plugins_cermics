...
subroutine get_j_k_dm(pd,ps,coul_d,exch_d,coul_s,exch_s)
 implicit none
 double precision, intent(in)  :: pd(ao_num, ao_num), ps(ao_num, ao_num)
 double precision, intent(out) :: coul_d(ao_num, ao_num), exch_d(ao_num, ao_num)
 double precision, intent(out) :: coul_s(ao_num, ao_num), exch_s(ao_num, ao_num)
 BEGIN_DOC
! J operator for P_d :: J(i,j) = \sum_{k,l} (ij|kl) P_d(k,l)
 END_DOC 
 use map_module
 ...
 double precision               :: integral, c0, c1, c2
 double precision               :: ao_two_e_integral, local_threshold
 double precision, allocatable  :: coul_s_tmp(:,:)
 double precision, allocatable  :: coul_d_tmp(:,:)
 double precision, allocatable  :: exch_s_tmp(:,:)
 double precision, allocatable  :: exch_d_tmp(:,:)
...
!!$OMP PARALLEL DEFAULT(NONE)                                      &
!    !$OMP PRIVATE(i,j,l,k1,k,integral,ii,jj,kk,ll,i8,keys,values,n_elements_max, &
!    !$OMP  n_elements,coul_s_tmp,coul_d_tmp,exch_s_tmp,exch_d_tmp)&
!    !$OMP SHARED(ao_num,SCF_density_matrix_ao_alpha,SCF_density_matrix_ao_beta,&
!    !$OMP  ao_integrals_map, coul_s, coul_d, pd, ps)

  call get_cache_map_n_elements_max(ao_integrals_map,n_elements_max)
  allocate(keys(n_elements_max), values(n_elements_max))
  allocate(coul_s_tmp(ao_num,ao_num), &
           coul_d_tmp(ao_num,ao_num))
  allocate(exch_s_tmp(ao_num,ao_num), &
           exch_d_tmp(ao_num,ao_num))
  coul_s_tmp = 0.d0
  coul_d_tmp  = 0.d0
  exch_s_tmp = 0.d0
  exch_d_tmp  = 0.d0
  ...
  do i8=0_8,ao_integrals_map%map_size
    ...
    do k1=1,n_elements
      ...
      do k2=1,8 ! loop over all 8 permutations for i,j,k,l
        ...
        l = ll(k2)
        integral = values(k1) 
        coul_d_tmp(i,j)  += pd(k,l) * integral 
        coul_s_tmp(i,j)  += ps(k,l) * integral 
        exch_s_tmp(l,j)  += ps(k,i) * integral
        exch_d_tmp(l,j)  += pd(k,i) * integral
      enddo
    enddo
  enddo
! !$OMP END DO NOWAIT
! !$OMP CRITICAL
  ...
  exch_d  += exch_d_tmp
! !$OMP END CRITICAL
  deallocate(keys,values,coul_s_tmp,coul_d_tmp)
  deallocate(exch_s_tmp,exch_d_tmp)
! !$OMP END PARALLEL
